...
exports.v2 = function (settings) {
    ...
    settings.includeRedirectUri =
        process.env.hasOwnProperty('INCLUDE_REDIRECT_URI') ? process.env.INCLUDE_REDIRECT_URI === 'true' :
        settings.hasOwnProperty('includeRedirectUri') ? settings.includeRedirectUri : true;
    ...
    return function (request, reply) {
        ...
        if (!request.query.code) {
            ...
            if (settings.includeRedirectUri) {
                debug('Redirect URI:', internals.location(request, protocol, settings.location));
            ...
            reply.state(cookie, state);
            debug('Setting cookie to', JSON.stringify(state, null, 2));
            debug('Redirecting to', settings.provider.auth + '?' + internals.queryString(query));
        ...
        debug('Requesting access_token, posting', JSON.stringify(query, null, 2));
        Wreck.post(settings.provider.token, requestOptions, function (err, tokenRes, payload) {
            ...
            debug('Getting a response', payload);

            if (err ||
                tokenRes.statusCode !== 200) {

                debug('Failed to get access_token');
                debug(' * status code :', tokenRes.statusCode);
